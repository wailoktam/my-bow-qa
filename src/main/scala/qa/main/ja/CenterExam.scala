...
object CenterExam {
  ...
  def main(args: Array[String]): Unit = {
    if (args.length < 4) {
      System.err.println("Usage: scala qa.main.ja.CenterExam MAX_SEARCH_RESULTS PROOF_THRESHOLD INPUT_XML OUTPUT_XML")
      System.exit(1)
    }
    ...
    val input_XML = XMLLoaderIgnoringDTD.loadFile(input_xml_name)
    val questions = ExtractQuestionsQ1000(input_XML)
    ...
    XML.save(output_xml_name, <questions>{ outputElems }</questions>, "UTF-8")
